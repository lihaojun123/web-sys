...
function dist(done) {
  rimraf.sync(path.join(cwd, 'cj-ui'))
...
function copyHtml() {
  ...
  rl.on('line', (line) => {
    ...
    const toPath1 = `cj-ui/components/${name}`
    const toPath2 = `cj-ui/components/${name}-cn`
    rimraf.sync(path.join(cwd, toPath1))
    rimraf.sync(path.join(cwd, toPath2))
    mkdirp(path.join(cwd, toPath1), function () {
      fs.writeFileSync(path.join(cwd, `${toPath1}/index.html`), fs.readFileSync(path.join(cwd, 'cj-ui/index.html')))
    })
    mkdirp(path.join(cwd, toPath2), function () {
      fs.writeFileSync(path.join(cwd, `${toPath2}/index.html`), fs.readFileSync(path.join(cwd, 'cj-ui/index.html')))
    })
  })
}

gulp.task('cj-ui', (done) => {
  dist(() => {
    copyHtml()
  })
})
